...
- Add Grok alongside Codex, Claude Code, and Gemini CLI, with portable startup
  from source ZIPs, empty first-run fields, and fictional demo screenshots.
- Remove private release artifacts and add automated release privacy checks.

- Refresh the SEO engine from public Claude SEO v2.2.5 and published maintenance
  at `a1480c7e`, retaining Dungeon's game, runtime choices, and direct API adapters.
- Update Codex agent profiles from the current portable specialist instructions.
- Package the managed Python runtime and Google update ledger in Codex installs.
- Preserve the exact upstream MIT notice and record public/Pro engine parity.

## [2.2] - 2026-06-18

### Refreshed - 2026-07-09
- **Google currency refresh**: updated the Google update ledger from the stale
  2026-05-25 verification state through official Google sources available on
  2026-07-09, including the June 2026 spam update, Merchant Center
  `video_link`, Search Console platform property reporting, and related
  Search Central documentation changes.
- **SEO reference corrections**: aligned Core Web Vitals threshold wording,
  `llms.txt` caveats, Soft Navigations status, local update history, schema-rich
  result caveats, and stale Google/DataForSEO/Firecrawl/SE Ranking/Ahrefs
  reference URLs.
- **Extension documentation parity**: filled DataForSEO cost/tool reference
  mirrors and documented optional extension `/seo <name>` command routing while
  preserving Codex as the packaged default and Claude Code/Gemini as opt-in
  local CLI choices.
- **Private-scope guardrail**: intentionally excluded upstream Claude SEO Brain
  marketing/workflow copy that conflicts with the Legends distribution and
  Codex-first positioning.

### Added
- **Codex-native SEO suite**: shipped the v2.2 SEO Dungeon workflow with Codex as the default local runtime, optional Claude Code and Gemini CLI paths, and a local WebSocket bridge for visible agent work.
- **Expanded SEO coverage**: added the 25-skill SEO suite for technical audits, content quality, schema, sitemaps, images, GEO, local SEO, backlinks, Google APIs, and DataForSEO-assisted research when credentials are configured.
- **Agentic browsing support**: added `llms.txt`, WebMCP metadata, and AI crawler access signals so agent-focused audits can discover the site and local workflow cleanly.
- **Direct API helpers**: added lightweight DataForSEO and Firecrawl scripts so live SEO data can run from project credentials without requiring MCP setup.

### Changed
- **Release identity**: consolidated the patch-level v2.2.x train into the public-facing `v2.2` release line to avoid confusion with the separate Claude SEO project versioning.
- **Env-first audit policy**: audit and chat prompts now prefer direct API/script access from project credentials, with MCP adapters treated as optional when already available.
- **Codex steering readiness**: remote-control and Guild Ledger flows now expose stronger receipts, queue state, and steering capability before automation injects follow-up prompts.

### Fixed
- **Proof and bridge reliability**: hardened desktop proof capture, bridge command safety, queue behavior, steering paths, audit fixture caching, and redacted failed-audit logs.
- **Launcher defaults**: aligned local app and bridge ports, default project paths, installer refs, and README/install docs with the v2.2 distribution path.

### Quality
- Validated across build, live bridge, remote-control, prompt policy, dialogue, hall, UX harness, and stress-test gates during v2.2 release preparation.
